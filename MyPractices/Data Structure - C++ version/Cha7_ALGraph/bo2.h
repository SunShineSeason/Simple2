...
#include"Queue\bo2.h"
   ...
   void BFSTraverse(){//使用队列来实现广度优先遍历
	    this->visited=new bool[this->vexnum];
		for(int i=0;i<this->vexnum;i++)
			this->visited[i]=false;
		Queue queue;int front;
		queue.InitQueue();
		cout<<this->graph[0].data<<"  ";  this->visited[0]=true;  queue.EnQueue(0);
	    while(!queue.Empty()){//队列不为空
			queue.DeQueue(front);
			ArcPointer pt=this->graph[front].firstarc;
			while(pt){
				if(!visited[pt->adjvex]){//未访问的顶点先访问后入队列，如果先入队列后访问的话会存在重复入队列的顶点被重复访问的问题!!
					cout<<this->graph[pt->adjvex].data<<"  ";
					this->visited[pt->adjvex]=true;
					queue.EnQueue(pt->adjvex);
				}	
				pt=pt->nextarc;
			}
		}
